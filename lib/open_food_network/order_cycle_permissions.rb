module OpenFoodNetwork
  ...
  class OrderCyclePermissions < Permissions
    ...
    def order_cycle_exchange_ids_distributing_my_variants
      ...
      variants = Spree::Variant.joins(:product).where('spree_products.supplier_id IN (?)', producer_ids)
      active_exchange_ids = @order_cycle.exchanges.outgoing.with_any_variant(variants.select("spree_variants.id")).pluck :id
