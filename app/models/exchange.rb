class Exchange < ActiveRecord::Base
  ...
  scope :involving, lambda { |enterprise_ids| where('exchanges.receiver_id IN (?) OR exchanges.sender_id IN (?)', enterprise_ids, enterprise_ids).select('DISTINCT exchanges.*') }
